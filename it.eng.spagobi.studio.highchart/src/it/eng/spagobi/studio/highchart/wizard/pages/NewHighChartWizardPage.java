 ...
 **/
package it.eng.spagobi.studio.highchart.wizard.pages;
...
public class NewHighChartWizardPage extends WizardPage {
	...
	private static Logger logger = LoggerFactory.getLogger(NewHighChartWizardPage.class);
	private IWorkbench workbench;

	public NewHighChartWizardPage( IWorkbench _workbench, String pageName) {
		super(pageName);
		setTitle("New high Chart ...");
	...
	public void createControl(Composite parent) {
		...
		try{
			//Type 
			final List chartTypes = HighChartConfigurations.getConfiguredChartTypes();

			if (chartTypes == null || chartTypes.size() == 0) {
				MessageDialog.openWarning(parent.getShell(), "Warning", "No Charts configured, see the ChartsInformation.xml file");
				return;
			}
			...
			for (Iterator iterator = chartTypes.iterator(); iterator.hasNext();) {
				...
				compImage.addPaintListener(new PaintListener() {
					public void paintControl(PaintEvent e) {
						...
						try {
							String imagePath=HighChartUtils.getChartImagePath(t.toUpperCase());
							InputStream is=HighChartUtils.getInputStreamFromResource(imagePath);
	...
	public static void setSelectedType(String selectedType) {
		NewHighChartWizardPage.selectedType = selectedType;
	}
